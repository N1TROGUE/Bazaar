    ...
    <form method="POST" action="{{ route('advertisements.store') }}" enctype="multipart/form-data">
        ...
        <div class="space-y-12">
            <div class="border-b border-gray-900/10 pb-12">
                ...
                @if(session('success'))
                    <div
                        class="p-4 mb-4 bg-green-100 border-l-4 border-green-500 text-green-700 rounded-lg w-full sm:col-span-6">
                ...
                <div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-3">
                        <label for="title" class="block text-sm/6 font-medium text-gray-900">Titel<span
                        ...
                        <div class="mt-2">
                            <div
                                ...
                                <input style="--tw-ring-color: {{ $appSettings->button_color ?? '#4f46e5' }};"
                                       type="text" name="title" id="title" placeholder="Advertentietitel"
                    ...
                    <!-- Beschrijving -->
                    <div class="sm:col-span-3">
                        <label for="description" class="block text-sm/6 font-medium text-gray-900">Beschrijving<span
                                style="color: red;"> *</span></label>
                        <div class="mt-2">
                    ...
                    <!-- Categorie -->
                    <div class="sm:col-span-3">
                        <label for="advertisement_category_id" class="block text-sm/6 font-medium text-gray-900">Categorie<span
                                style="color: red;"> *</span></label>
                        <div class="mt-2">
                            <div
                                class="flex items-center rounded-md bg-white outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                                <select style="--tw-ring-color: {{ $appSettings->button_color ?? '#4f46e5' }};"
                                    ...
                                    @foreach ($categories as $category)
                                        <option
                                            value="{{ $category->id }}" {{ old('advertisement_category_id') == $category->id ? 'selected' : '' }}>
                    ...
                    <!-- Type advertentie -->
                    <div class="sm:col-span-3">
                        <label for="ad_type" class="block text-sm/6 font-medium text-gray-900">Type advertentie<span
                                style="color: red;"> *</span></label>
                        <div class="mt-2">
                            <div
                                class="flex items-center rounded-md bg-white outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                    ...
                    <!-- Prijs -->
                    <div class="sm:col-span-3">
                        <label for="price" class="block text-sm/6 font-medium text-gray-900">Prijs (€)<span
                                style="color: red;"> *</span></label>
                        <div class="mt-2">
                            <div
                                class="flex items-center rounded-md bg-white outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                    ...
                    <div class="col-span-full">
                        <label for="image_path" class="block text-sm/6 font-medium text-gray-900">Afbeelding
                            uploaden<span style="color: red;"> *</span></label>
                        <div
                            class="mt-2 flex justify-center w-1/2 rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                    ...
                    <div class="sm:col-span-full w-1/2">
                        <label for="related_advertisements" class="block text-sm/6 font-medium text-gray-900">Gerelateerde
                            advertenties (optioneel)</label>
                        <div class="mt-2">
                            <select style="--tw-ring-color: {{ $appSettings->button_color ?? '#4f46e5' }};"
                                ...
                                @if(isset($allAdvertisements) && $allAdvertisements->isNotEmpty())
                                    @foreach ($allAdvertisements as $ad)
                                        <option
                                            value="{{ $ad->id }}" {{ (is_array(old('related_advertisements')) && in_array($ad->id, old('related_advertisements'), true)) ? 'selected' : '' }}>
                                            {{ $ad->title }}
                                        </option>
                                    @endforeach
                                @else
                                    <option value="" disabled>Geen advertenties beschikbaar.</option>
                                @endif
                            </select>
                            <p class="mt-1 text-xs text-gray-500">Houd Ctrl (of ⌘ op Mac) ingedrukt om meerdere
                                advertenties te selecteren.</p>
                        </div>
                        @error('related_advertisements')
                        <x-form-error>{{ $message }}</x-form-error>
                        @enderror
                        @error('related_advertisements.*') {{-- Error for individual items in the array --}}
                        <x-form-error>{{ $message }}</x-form-error>
                        @enderror
                    </div>
                    ...
                    <!-- Minimale huurduur -->
                    <div class="sm:col-span-3">
                        <label for="rental_min_duration_hours" class="block text-sm/6 font-medium text-gray-900">Min.
                            huur (uren) <span style="color: red;"> in te vullen bij verhuur</span></label>
                        <div class="mt-2">
                            <div
                                class="flex items-center rounded-md bg-white outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                                <input style="--tw-ring-color: {{ $appSettings->button_color ?? '#4f46e5' }};"
                    ...
                    <!-- Maximale huurduur -->
                    <div class="sm:col-span-3">
                        <label for="rental_max_duration_hours" class="block text-sm/6 font-medium text-gray-900">Max.
                            huur (uren) <span style="color: red;"> in te vullen bij verhuur</span></label>
                        <div class="mt-2">
                            <div
                                class="flex items-center rounded-md bg-white outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
