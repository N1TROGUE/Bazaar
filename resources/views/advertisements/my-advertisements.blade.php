        ...
        <div class="pb-12">
             <form method="GET" class="mb-6">
                    <div class="flex flex-wrap justify-between items-end gap-4">
                        ...
                        {{-- Filter knop --}}
                        <div class="ml-auto">
                            <button type="submit" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                Filter
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-3 flex flex-col justify-between">
                        <div>
                            ...
                            @if(now()->gt($advertisement->expiration_date))
                                <span class="inline-block mt-1 text-xs font-semibold text-red-800 bg-red-100 px-2 py-0.5 rounded-full">Verlopen</span>
                            @elseif(now()->diffInDays($advertisement->expiration_date) <= 3)
                                <span class="inline-block mt-1 text-xs font-semibold text-orange-800 bg-orange-100 px-2 py-0.5 rounded-full">Loopt bijna af</span>
                            @else
                                <span class="inline-block mt-1 text-xs font-semibold text-green-800 bg-green-100 px-2 py-0.5 rounded-full">Actief</span>
                            @endif
                        </div>
