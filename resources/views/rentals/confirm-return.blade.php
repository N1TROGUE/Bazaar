<x-layout>
    <x-slot:heading>
        {{ __('confirmreturns.return_confirm') }}: {{ $rental->advertisement->title }}
    </x-slot:heading>
        ...
        <div class="bg-white shadow-2xl rounded-xl overflow-hidden border border-gray-300">
            <div class="md:flex">
                ...
                <div class="md:w-1/2 p-6 lg:p-8">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">{{ __('confirmreturns.product_details') }}</h2>
                    ...
                    @else
                        <div class="aspect-square w-full h-64 bg-gray-200 flex items-center justify-center rounded-lg text-gray-400 mb-4">
                            {{ __('confirmreturns.no_image_available') }}
                        </div>
                    @endif
                    <p class="text-lg text-gray-700"><span class="font-medium">{{ __('confirmreturns.product') }}:</span> {{ $rental->advertisement->title }}</p>
                    <p class="mt-1 text-sm text-gray-600"><span class="font-medium">{{ __('confirmreturns.borrowed_on') }}:</span> {{ $rental->rented_from->format('d-m-Y') }}</p>
                    <p class="mt-1 text-sm text-gray-600"><span class="font-medium">{{ __('confirmreturns.return_by') }}:</span> {{ $rental->rented_until->format('d-m-Y') }}</p>
                </div>

                {{-- Rechts: Retour formulier --}}
                <div class="md:w-1/2 p-6 lg:p-8 bg-gray-50 border-t md:border-t-0 md:border-l border-gray-200 flex flex-col justify-between">
                    <div>
                        <h3 class="text-xl font-semibold text-gray-900 mb-6">{{ __('confirmreturns.return_info') }}</h3>
                        ...
                        <form action="{{ route('rentals.return', $rental) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div>
                                <div class="space-y-5">
                                    <div>
                                        <label for="return_image" class="block text-sm font-medium text-gray-700 mb-1">{{ __('confirmreturns.upload_return_image') }}</label>
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">{{ __('confirmreturns.image_upload_note') }}</p>
                                    </div>
                                </div>
