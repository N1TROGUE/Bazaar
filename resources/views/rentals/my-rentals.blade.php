...
<x-layout>
    ...
    @if($rentals->isEmpty())
        <p>Je hebt momenteel geen verhuringen.</p>
    @else
        <div class="space-y-4">
            <form method="GET" class="mb-6">
                <div class="flex flex-wrap justify-between items-end gap-4">
                    ...
                    {{-- Prijs sortering --}}
                    <div>
                        <label for="sort_price" class="block text-sm font-medium text-gray-700 mb-1">Sorteer op
                            prijs</label>
                        <select name="sort_price" id="sort_price"
                                class="w-56 rounded-lg border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="">-- Geen sortering --</option>
                            <option value="asc" @if(request('sort_price') === 'asc') selected @endif>Laag naar hoog
                            </option>
                            <option value="desc" @if(request('sort_price') === 'desc') selected @endif>Hoog naar laag
                            </option>
                        </select>
                    </div>
